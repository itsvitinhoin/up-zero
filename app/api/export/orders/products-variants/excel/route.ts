...
export async function GET(request: NextRequest) {
  ...
  const response = await fetch(`${baseUrl}/orders/export/products-variants?${params.toString()}`, {
    method: 'GET',
    headers: {
  ...
  if (!response.ok) {
    ...
    return NextResponse.json(
      { error: `Erro ao exportar produtos vendidos por variante: ${response.status} ${text}` },
  ...
  return new NextResponse(buffer, {
    ...
    headers: {
      ...
      'Content-Disposition': `attachment; filename=produtos-vendidos-variantes-${dateStr}.xlsx`,
    },
  })
}
