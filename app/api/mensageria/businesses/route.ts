...
export async function GET() {
  ...
  if (!result.ok) {
    ...
    return NextResponse.json({ data: (await getState()).businesses, error: result.error, source: 'local' })
  }
  ...
  await addLog({ type: 'business_loaded', status: 'success', description: 'Business Managers loaded from Meta.', safePayload: { count: result.data?.length ?? 0 } })
