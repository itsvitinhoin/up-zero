...
export function FacebookOAuthButton({ onSuccess, isReconnect = false, className }: Props) {
  ...
  // ── Load + initialize FB SDK ──────────────────────────────────────────────
  useEffect(() => {
    if (!appId || step === 'no-config') return
    ...
    if (existing) {
      existing.addEventListener('error', () => {
        if (cancelled) return
        setError(
          'Facebook SDK foi bloqueado no navegador. Verifique Content-Security-Policy, extensoes de bloqueio (AdBlock/uBlock/Brave Shields) ou proxy/firewall para connect.facebook.net.',
        )
        setStep('error')
      }, { once: true })
      return () => { cancelled = true }
