...
export default function AdminAbandonedCartDetailRoute({
  params,
}: {
  ...
  return (
    <Suspense fallback={<AdminRouteSkeleton />}>
      <AdminAbandonedCartDetailRouteContent params={params} />
    </Suspense>
  );
}

async function AdminAbandonedCartDetailRouteContent({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.warn("Admin abandoned cart detail endpoint unavailable", {
    ...
    return <AdminAbandonedCartDetailPage cart={cart} basePath="/admin/carrinhos-abandonados" />;
  } catch (error) {
    console.error("Failed to fetch admin abandoned cart detail:", error);
    return notFound();
  }
}
