                ...
                {settingsNav.map((group) => {
                  ...
                  if (group.items.length > 0) {
                    return (
                      <div
                        key={group.key}
                        className={cn("block rounded-2xl border border-transparent px-2 py-1 transition-colors hover:bg-muted/50")}
                      >
                        <Link href={group.href} className="block rounded-xl px-3 py-3 hover:bg-muted/70 transition-colors">
                          <div className="flex items-center gap-3 font-medium">
                            <Icon className={cn("h-5 w-5 shrink-0", isActive ? "text-primary" : "text-muted-foreground")} />
                            <span className={cn("text-sm", isActive ? "text-primary font-semibold" : "text-foreground")}>{group.label}</span>
                          </div>
                        </Link>
                  ...
                  return (
                    <Link
                      ...
                      href={group.href}
                      className={cn(
                        "block rounded-2xl border border-transparent px-2 py-1 transition-colors hover:bg-muted/50",
                        isActive ? "bg-muted/40" : ""
                      )}
                    >
                      <div className="flex items-center gap-3 font-medium px-3 py-3">
                        <Icon className={cn("h-5 w-5 shrink-0", isActive ? "text-primary" : "text-muted-foreground")} />
                        <span className={cn("text-sm", isActive ? "text-primary font-semibold" : "text-foreground")}>{group.label}</span>
                      </div>
                    </Link>
